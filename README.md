...
1. В чем заключается алгоритм частотного анализа?
  символы сортируются по частоте и меняются на символы
  примера незашифрованного текста,
  тоже сортированные по частоте. В случае шифра Цезаря достаточно просто найти
  самый частый символ и вычесть из него пробел, восстановив тем самым ключ
2. Какие есть другие распространенные атаки на криптографические алгоритмы?
  brute force, интерполяция и другие
3. Какие симметричные шифры используются в настоящее время и считаются надежными?
  AES, Serpent и другие
